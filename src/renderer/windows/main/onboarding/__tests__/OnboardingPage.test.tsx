...
async function openProviderSetup() {
  fireEvent.click(screen.getByRole('button', { name: /settings\.provider\.add\.button_title/ }))
...
describe('OnboardingPage', () => {
  ...
  it('shows provider setup with onboarding mode when adding a provider', async () => {
  ...
  it('blocks the fresh install with zero providers from leaving provider setup', async () => {
    enabledProvidersMock.splice(0)
  ...
  it('renders the model step with autofill, a selection callback and compact presentation', async () => {
    selectedModelsMock.translateModel = undefined
    render(<OnboardingPage />)
    ...
    expect(modelSettingsProps?.compact).toBe(true)
    expect(modelSettingsProps?.showSettingsButton).toBe(false)
    expect(modelSettingsProps?.showDescription).toBe(false)
    expect(modelSettingsProps?.showDividers).toBe(false)
    expect(modelSettingsProps).not.toHaveProperty('modelFilter')
    expect(screen.getByRole('button', { name: /onboarding\.select_model\.start/ })).toBeDisabled()
  })

  it('binds the first chosen default model to the seeded model-less assistant', async () => {
    dataApiMocks.get.mockImplementation(async (path: string) => {
      if (path === '/assistants') {
        return {
          items: [{ id: 'assistant-1', modelId: null }],
          total: 1
        }
      }
  ...
  it('leaves an assistant that already has a model untouched', async () => {
    dataApiMocks.get.mockImplementation(async (path: string) => {
      if (path === '/assistants') {
        return {
          items: [{ id: 'assistant-1', modelId: 'openai::gpt-4o-mini' }],
          total: 1
        }
      }
      throw new Error(`Unexpected path: ${path}`)
  ...
  it('preserves assistant models unless the library holds exactly one assistant', async () => {
    dataApiMocks.get.mockImplementation(async (path: string) => {
      if (path === '/assistants') {
        return {
          items: [
  ...
  it('opens provider setup without privacy acceptance', async () => {
    MockUsePreferenceUtils.setPreferenceValue('app.privacy.policy_version', '')
    render(<OnboardingPage />)
  ...
  it('skips onboarding without privacy acceptance', async () => {
    MockUsePreferenceUtils.setPreferenceValue('app.privacy.policy_version', '')
    render(<OnboardingPage />)
  ...
  it('stays on the welcome page when saving privacy acceptance fails', async () => {
    ...
    expect(screen.getByRole('button', { name: 'settings.provider.add.button_title' })).toBeInTheDocument()
  })
  ...
  it('uses an elevated welcome layout with clear text hierarchy and intentional spacing', () => {
    ...
    const primaryAction = screen.getByRole('button', { name: 'settings.provider.add.button_title' })
  ...
  it('offers the add-provider action as the only way forward from the welcome step', () => {
    render(<OnboardingPage />)

    const welcomeActions = screen.getByRole('button', { name: 'settings.provider.add.button_title' }).parentElement
    expect(welcomeActions?.querySelectorAll('button')).toHaveLength(1)
  })
})
