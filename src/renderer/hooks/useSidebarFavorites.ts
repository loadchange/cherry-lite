 ...
 *
 * `favorites` is the full ordered list the sidebar renders and drag-reorders;
 * `reorderFavorites` persists a new order. `appFavorites` projects the app ids for
 * surfaces (launchpad) that need pin state, and `setAppPinned` mutates membership.
 * The launchpad owns its own tile ordering via `ui.launchpad.app_order`, so
 * favorites carries the sidebar order only. Every mutation goes through the
 * helpers in `utils/sidebar`, so components never touch the raw `type` tags.
 */
