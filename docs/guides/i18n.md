...
本仓库只发布两种界面语言：`zh-CN`（`src/renderer/i18n/locales/zh-cn.json`）和 `en-US`（`en-us.json`）。不要再加其它 locale 文件。
...
This script uses `zh-cn.json` as the source of truth to sync structure into `en-us.json`, including:
...
Typically, after adding new texts in `zh-cn.json`, run `i18n:sync`, then `i18n:translate` to fill `en-us.json`.
...
3. Run `pnpm i18n:sync` to copy keys into `en-us.json`
4. Run `pnpm i18n:translate` to fill English placeholders
5. Review the English strings before committing

## Best Practices

1. **Use Chinese as Source Language**: 先写 `zh-cn.json`，再同步到 `en-us.json`。
